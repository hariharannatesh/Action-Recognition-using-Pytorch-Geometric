	...
	def forward(self,batch,kernel,stride):
		...
		for i,data_graph in enumerate(listt):
			...
			if(people==1):
				...
				x = data_graph.x 
				x,edge_index_new = Sample_data.forward(self,x,kernel,stride)

				data_graph.x = x
				data_graph.edge_index = edge_index_new.to(data_graph.x.device)
				listt[i] = data_graph
				...
				x1,edge_index1 = Sample_data.forward(self,x1,kernel,stride)
				x2,edge_index2 = Sample_data.forward(self,x2,kernel,stride)

				data_1 = Data(x=x1,edge_index=edge_index1)
				data_2 = Data(x=x2,edge_index=edge_index2)
				...
				data_graph.edge_index = mini_batch.edge_index.to(data_graph.x.device)
				listt[i] = data_graph
