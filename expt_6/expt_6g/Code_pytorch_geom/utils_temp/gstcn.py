...
class Net_time(nn.Module):
	def __init__(self,
		...
		self.edge_conv = nn.Conv1d(1,1,kernel_size=kernel,padding=(kernel-1)//2,stride=1)
	...
	def forward(self,data):
		...
		convolved_edge_weight = convolved_edge_weight.view(lent)

		data.x=data.x.permute(1,0,2).contiguous()
		data.x=self.tcnconv(data.x,edge_index_t,edge_weight=convolved_edge_weight)
		data.x=data.x.permute(1,0,2).contiguous()
