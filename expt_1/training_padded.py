	...
	def forward(self,data,stride=1,kernel=9,residual=False):
		...
		if(residual):
			if(self.inc==self.outc):
				...
				# print("X res before bn contiguous",x_res.is_contiguous(memory_format=torch.contiguous_format))
				x_res=self.bn_2d(x_res.double())
				x_res=x_res.permute(0,2,3,1).contiguous()
				# print("X res after bn contiguous",x_res.is_contiguous(memory_format=torch.contiguous_format))
		...
		# print("X spatial contiguous",x.is_contiguous(memory_format=torch.contiguous_format))
		x=self.bn_2d(x)
		x=self.relu(x)
		x=x.permute(0,2,3,1).contiguous()
		# print("X spatial after relu contiguous",x.is_contiguous(memory_format=torch.contiguous_format))
		...
		# print("X before temporal  contiguous",x.is_contiguous(memory_format=torch.contiguous_format))
		x=self.conv1(x,edge_index_t,edge_weight=edge_weight)
		x=x.permute(0,2,1,3).contiguous()
		# print("X after temporal  contiguous",x.is_contiguous(memory_format=torch.contiguous_format))
		x=x.permute(0,3,1,2).contiguous()
		# print("X after temporal before bn contiguous",x.is_contiguous(memory_format=torch.contiguous_format))
		x=self.bn_2d(x)
		x=self.drop(x)
		x=x.permute(0,2,3,1).contiguous()
		# print("X after bn,dropping contiguous",x.is_contiguous(memory_format=torch.contiguous_format))
		...
		data_final.x=self.relu(data_final.x)
...
class Model(nn.Module):
	...
	def forward(self,data):
		...
		data.x=data.x.to(memory_format=torch.channels_last)

		# print("Input data x",data.x.is_contiguous(memory_format=torch.contiguous_format))

		x=data.x.permute(0,2,3,1).contiguous()
		# print("X input ",x.is_contiguous(memory_format=torch.contiguous_format))
		M,V,C,T=x.size()
		x=x.view(M,V*C,T)
		# print("X before 1d bn contiguous",x.is_contiguous(memory_format=torch.contiguous_format))
		x=self.bn_1d(x)

		x=x.view(M,V,C,T)
		# print("X after 1d bn",x.is_contiguous(memory_format=torch.contiguous_format))
		x=x.permute(0,3,1,2).contiguous()
		# print("X after 1d bn reshaped contiguous",x.is_contiguous(memory_format=torch.contiguous_format))
		data.x=x 
		...
		# print("X result contiguous",x_result.is_contiguous(memory_format=torch.contiguous_format))

		x_avg=F.avg_pool2d(x_result,x_result.size()[2:]).cuda()
		x_avg=x_avg.contiguous(memory_format=torch.contiguous_format)

		# print("X avg contiguous",x_avg.is_contiguous(memory_format=torch.channels_last))

		op.batch=op.batch.cuda()	

		# print("Batch",op.batch)

		x_mean=GMP(x_avg,batch=op.batch).cuda()
		x_mean=x_mean.contiguous(memory_format=torch.contiguous_format)

		# print("X mean contiguous",x_mean.is_contiguous(memory_format=torch.channels_last))

		predicted_val=self.fcn(x_mean)
		# print("Predicted value size",predicted_val.size())
		predicted_val=predicted_val.view(predicted_val.size(0),-1)
