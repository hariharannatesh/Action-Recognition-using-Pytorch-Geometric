...
for f in files:
	...
	if (classs<=17):
		...
		data_new=np.zeros((shape[0],shape[1],shape[2]*2))

		if('skel_body1' in keys):
			data_new[:,:,0:3]=data['skel_body0']
			data_new[:,:,3:6]=data['skel_body1']
		...
		np.save('/ssd_scratch/cvit/nateshhariharan/Dataset_NTU_RGBD/ntu60_numpy_featurepadded/'+name+'.npy',data_new)
